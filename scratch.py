...
for i in range(1,ROWS):
    actual_grid[0][i] = BLACK
for i in range(0, ROWS-1):
    actual_grid[i][ROWS-1] =BLACK 
    actual_grid[i][ROWS-5] =BLACK
...
h = [[hcost([i,j]) for j in range(ROWS)] for i in range(ROWS)]
...
def astar_agent(start):
    ...
    if start==(1,0):
        print('---------------------------------')
        print('CALCULATING PATH FROM:', start)
        #print(path)
        print(openset.queue)
    ...
    while not openset.empty():
        ...
        if start==(1,0):
            ...
            print('CHOICE:', choice)
            print('NEIGHBORS: ',give_neighbors_agent(choice))
